...
use crate::{span, Event, LevelFilter, Metadata, Dispatch};
...
pub trait Collect: 'static {
    ...
    fn drop_span(&self, _id: span::LocalId) {}
...
/// [`try_close`]: Collect::try_close
/// [cs-reg]: crate::callsite#registering-callsites
pub trait GlobalCollect: 'static {
    // === Span registry methods ==============================================
    ...
    /// [span ID]: super::span::LocalId
    /// [`try_close`]: Collect::try_close
    fn clone_span(&self, id: &span::GlobalId) -> span::GlobalId {
        id.clone()
    }
    ...
    /// [`try_close`]: Collect::try_close
    #[deprecated(since = "0.1.2", note = "use `Collector::try_close` instead")]
    fn drop_span(&self, _id: span::GlobalId) {}
    ...
    /// [`clone_span`]: Collect::clone_span
    /// [`drop_span`]: Collect::drop_span
    fn try_close(&self, id: span::GlobalId) -> bool {
        #[allow(deprecated)]
        self.drop_span(id);
        false
    }
...
impl<C> GlobalCollect for C
where
    C: 'static + Collect + Send + Sync
{
    fn register_callsite(&self, metadata: &'static Metadata<'static>) -> Interest {
        Collect::register_callsite(self, metadata)
    }

    fn enabled(&self, metadata: &Metadata<'_>) -> bool {
        Collect::enabled(self, metadata)
    }

    fn max_level_hint(&self) -> Option<LevelFilter> {
        Collect::max_level_hint(self)
    }

    fn new_span(&self, span: &span::Attributes<'_>) -> span::GlobalId {
        crate::dispatch::get_default(|dispatch| {
            assert!(<Dispatch as PartialEq<C>>::eq(dispatch, &self));
            let local_id = Collect::new_span(self, span);
            span::GlobalId::new(local_id, dispatch.clone())
        })
    }

    fn record(&self, span: &span::GlobalId, values: &span::Record<'_>) {
        Collect::record(self, &span.local_id(), values)
    }

    fn record_follows_from(&self, span: &span::GlobalId, follows: &span::GlobalId) {
        Collect::record_follows_from(self, &span.local_id(), &follows.local_id())
    }

    fn event(&self, event: &Event<'_>) {
        Collect::event(self, event)
    }

    fn enter(&self, span: &span::GlobalId) {
        Collect::enter(self, &span.local_id())
    }

    fn exit(&self, span: &span::GlobalId) {
        Collect::exit(self, &span.local_id())
    }

    fn clone_span(&self, span: &span::GlobalId) -> span::GlobalId {
        span.with_local_id(Collect::clone_span(self, &span.local_id()))
    }

    fn drop_span(&self, span: span::GlobalId) {
        #[allow(deprecated)]
        Collect::drop_span(self, span.local_id())
    }

    fn try_close(&self, span: span::GlobalId) -> bool {
        Collect::try_close(self, span.local_id())
    }

    fn current_span(&self) -> span::Current {
        Collect::current_span(self)
    }
}

impl dyn GlobalCollect {
    /// Returns `true` if this `Collector` is the same type as `T`.
    pub fn is<T: Any>(&self) -> bool {
        self.downcast_ref::<T>().is_some()
    }
...
/// Use the given collector for the duration of the entire program.
///
/// Can only be set once; subsequent attempts to set the global default will fail.
/// Returns whether the initialization was successful.
///
/// **Note:** Libraries should *NOT* call `set_global_default()`! That will
/// cause conflicts when executables try to set them later.
#[cfg(feature = "alloc")]
#[cfg_attr(docsrs, doc(cfg(any(feature = "std", feature = "alloc"))))]
pub fn set<C>(collector: C) -> Result<(), crate::dispatch::SetGlobalDefaultError>
where
    C: Collect + Send + Sync + 'static,
{
    crate::dispatch::set(crate::Dispatch::new(collector))
}